...
test('Cmd+K opens the palette; typing filters and Enter runs the command', async () => {
  ...
  // the terminal slice's three font-zoom commands + the app-wide shortcut commands + the PR Review
  // Inbox slice's prInbox.sync / prInbox.review + the Sessions slice's sessions.refresh + the My
  // Work slice's myWork.refresh + the Time Tracking slice's timeTracking.refresh), plus the one
  // workspace this test just added as a switch-to target. The nine positional tab jumps and the
  // palette's own open command are mapped but deliberately not listed here.
  await openPalette(app)
  await expect(win.locator('.ix-palette')).toBeVisible()
  await expect(win.locator('.ix-palette__item')).toHaveCount(26)
